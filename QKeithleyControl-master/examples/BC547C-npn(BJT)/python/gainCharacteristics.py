...
if __name__ == "__main__":
	...
	# Path to data files
	path = "../dat/BJT-BC547C(npn)-CE-ice(ibe).dat"
	...
	for _key in data.keys():
		...
		if data.get_key_data(_key) != {}:
			...
			# plot the data
			ax0.plot( [ ibe_unit * _ for _ in ibe[2:] ], beta[2:], color = cmap.to_rgba(vce) )


	# Add axes lables and show plot
	#ax0.set_title("BJT BC547C(npn) : Common Emitter")
	ax0.set_title("BJT BC547C(npn) : Gain  Characteristics")
